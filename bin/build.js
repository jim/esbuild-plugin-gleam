...
export async function build () {
  const { name, gleamDependencies } = JSON.parse(
    await readFile('./package.json')
  );

  await Promise.all(gleamDependencies.map(clone));
  for (const dep of gleamDependencies) await cachedBuildProject(dep);
...
async function buildProject ({ name, root, dependencies, includeTests }) {
  ...
  const dir = root || libraryDir(name);
...
async function test () {
  const gleamPackage = await build();

  console.log('Running tests...');

  const dir = `target/lib/${gleamPackage.name}`;
  let passes = 0;
  let failures = 0;

  for await (const path of await getFiles(dir)) {
    if (!path.endsWith('_test.js')) continue;
    const module = await import(path);

    for await (const fnName of Object.keys(module)) {
...
async function start () {
  ...
  const { main } = await import(join(outDir(name), 'main.js'));
  return main();
}
